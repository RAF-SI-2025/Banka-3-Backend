...
func TestCreateAccountWithCardSuccess(t *testing.T) {
	server, mock, db := newTestServer(t)
	defer func() { _ = db.Close() }()
	...
	cardNumber, err := GenerateCardNumber(visa, accountNumber)
	if err != nil {
		t.Fatalf("GenerateCardNumber: %v", err)
	}
	...
	mock.ExpectQuery("INSERT INTO cards").
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "number", "type", "brand", "creation_date", "valid_until", "account_number", "cvv", "card_limit", "status"}).
			AddRow(int64(77), cardNumber, "debit", "visa", createdAt, time.Now().AddDate(5, 0, 0), accountNumber, "123", float64(0), "active"))
	mock.ExpectCommit()
	...
	if resp.AccountNumber != accountNumber {
		t.Fatalf("unexpected account number: %s", resp.AccountNumber)
	}
